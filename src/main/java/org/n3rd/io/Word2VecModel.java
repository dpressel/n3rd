...
//                int oldpos = rdr.position();
//                rdr.position(oldpos+(4*(int) layerSize));

                // remap file
                if (rdr.position() > BUFFER)
                {
                    long temp = channel.size();
    ...
    public float[] getVec(String word){
        if(this.getVocab().containsKey(word)){
            return this.vectors[this.getVocab().get(word)];
        }
        return this.NULLV;
